...
def main(
    ...
    for j in range(0, len(inputList), pageSize):
        ...
        for i in trange(start, end, batchSize):
            ...
            if True:
                IDstring = " ".join(["'" + q.upper() + "'" for q in IDs])
            else:
                IDstring = " ".join(["wd:" + q.upper() + "" for q in IDs])
        ...
        if batch:
            df.to_csv(outputFileDir + "-" + batch + ".csv", index=False)
        else:
            df.to_csv(
                outputFileDir + "-" + str(start) + "-" + str(end) + ".csv",
                index=False,
            )
